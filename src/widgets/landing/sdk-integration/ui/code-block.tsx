...
const CodeBlock: React.FC<CodeBlockProps> = ({ tabs, githubUrl }) => {
    ...
    return (
        <div className="bg-a-bg-website border border-gray-200 rounded-2xl overflow-hidden shadow-none! w-full">
                ...
                <TabsList className="w-full justify-start bg-transparent border-b border-gray-200 rounded-none h-auto p-0">
                    {tabs.map((tab) => (
                        <TabsTrigger
                            key={tab.label}
                            value={tab.label}
                            className="px-8 py-4 rounded-none border-b-2 border-transparent data-[state=active]:border-b-gray-900 data-[state=active]:bg-transparent bg-transparent text-gray-500 data-[state=active]:text-gray-900 data-[state=active]:shadow-none font-medium"
                        >
                            {tab.label}
                        </TabsTrigger>
                    ))}
                    ...
                    <TabsContent key={tab.label} value={tab.label} className="mt-0">
                        <div className="relative">
                            <Button
                                variant="ghost"
                                size="icon"
                                className="absolute top-4 right-4 text-gray-600 hover:text-gray-900 hover:bg-gray-100 z-10"
                                onClick={handleCopy}
                            >
                                {copied ? (
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex items-center gap-3">
                    <svg viewBox="0 0 24 24" className="h-7 w-7 text-gray-900 fill-current">
                    ...
                    <span className="text-gray-600">100% Open Source</span>
                </div>
